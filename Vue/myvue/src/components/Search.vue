<template>
  <section>
    <div class="search">
      ...
      <div class="mapcity" v-show="show">
        <p>请选择你所在的城市 <em>选城市 > 定位置 > 叫外卖</em></p>
        <p><span>猜你在</span>
          <button @click="beDefault">杭州</button>
          | <input type="text" placeholder="请输入城市" v-model="word" @change="autoTip" @keypress.enter="enterHandler"></p>
        <wordlist :list="list" @citychange="getval"></wordlist>
      </div>
    </div>
  </section>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      enterHandler () {
        this.getval(this.word)
      },
      beDefault (event) {
        this.getval(event.target.innerText)
      },
      autoTip () {
        console.log(1)
      }
